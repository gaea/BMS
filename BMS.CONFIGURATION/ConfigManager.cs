...
namespace BMS.CONFIGURATION
{
    public class ConfigManager
    {
        ...
        private static string getKey(string key)
        {
            ...
            {
                throw new Exception(string.Format("La llave [{0}] del archivo de configuración es inválida, por favor realice la verificación respectiva", key), exc);
            }
        }
        ...
        private static void loadConfiguration()
        {
            try
            {
                ...
                fileMap.ExeConfigFilename = Assembly.GetCallingAssembly().Location;

                config = ConfigurationManager.OpenMappedExeConfiguration(fileMap, ConfigurationUserLevel.None);
            }
            catch (Exception exc)
            {
                throw new Exception(string.Format("Se presentaron inconvenientes realizando la carga del archivo de configuración [{0}], por favor realice la verificación respectiva", config.FilePath));
            }
